...
const {
  GrassManagementOptionsMapper,
} = require("../constants/grass-management-options-mapper");
const {
  SoilNitrogenMapper,
} = require("../constants/soil-nitrogen-supply-mapper");
const {
  SwardManagementMapper,
} = require("../constants/sward-management-mapper");
...
} = require("../db/entity/soil-nitrogen-supply-items.entity");
...
const calculatePreviousGrassLookupMethods = {
  ...
  async addOrganicNitrogenTotals(
    managementPeriodId,
    totals,
    transactionalManager,
  ) {
  ...
  async addFertiliserNitrogenTotal(
    managementPeriodId,
    totals,
    transactionalManager,
  ) {
  ...
  async getRb209CountryId(fieldId, transactionalManager) {
    if (!fieldId) {
      return null;
    }
    ...
    const country = await transactionalManager.findOne(CountryEntity, {
      where: { ID: farm.CountryID },
      select: ["RB209CountryID"],
    });

    return country?.RB209CountryID ?? null;
  },

  async getScotlandPreviousGrassId(crop, transactionalManager, harvestYear) {
    if (!crop?.FieldID) {
      return null;
    }

    if (crop.PreviousGrass != null) {
      return crop.PreviousGrass;
    }
    ...
    return previousCropping?.PreviousGrassID ?? null;
  },
  ...
  async getPreviousGrassID(crop, transactionalManager, harvestYear) {
    ...
    if (rb209CountryId === CountryMapper.SCOTLAND) {
      return this.getScotlandPreviousGrassId(
        crop,
        transactionalManager,
        harvestYear,
      );
